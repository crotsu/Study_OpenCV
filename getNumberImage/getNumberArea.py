...
# 1枚108文字，1人5枚で108*5=540文字
# 130人に頼めば70000文字を集めることができる．
# 情報工学科の学生全員に頼もう．
...
for filenumber in range(5):
    ...
    img = cv2.imread('../image/' + username + str(filenumber+1) + '.jpg', 1)
    ...
    df = pd.read_csv('../numberSheet/rand' + str(filenumber+1) + '.csv', header=None)
    ...
    for (i, pt) in enumerate(corner):
        ...
        if i<8:
            filename = '../result/' + username + '_' + str(filenumber) + '_date' + str(i) + '.jpg'
        elif i<16:
            filename = '../result/' + username + '_' + str(filenumber) + '_name' + str(i-8) + '.jpg'
        else:
            filename = '../result/' + username + '_' + str(filenumber) + '_' + str(i-16) + '.jpg'

        cv2.imwrite(filename, dst)

    # 教師信号を出力
    f = open('../result/' + str(filenumber) + '_teach.dat', 'w')
    for col in range(12):
        for row in range(9):
